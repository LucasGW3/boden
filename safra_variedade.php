...
<head>
  ...
  <style>
    ...
    /* antes .mini h3 era pequeno; agora os títulos recebem classe .var-title */
    .mini canvas { max-height: 560px; }
    .wrap-max { max-width: 2200px; }

    /* Destaque dos títulos das variedades */
    .var-header{ display:flex; align-items:baseline; justify-content:space-between; margin-bottom:.75rem; }
    .var-title{
      position:relative;
      padding-left: .85rem;
      font-weight: 900;
      /* responsivo: entre 1.25rem e 1.85rem */
    ...
    .var-title::before{
      ...
      background:#5FB141;
      box-shadow: 0 0 0 3px rgba(95,177,65,.18);
    }
...
(() => {
  ...
  const peForVar = (varName) => {
    const pe = PE_POR_VAR[varName] || {};
    return {
      colheita:    Number.isFinite(pe.colheita)    ? pe.colheita    : DEFAULT_PE.colheita,
      beneficiado: Number.isFinite(pe.beneficiado) ? pe.beneficiado : DEFAULT_PE.beneficiado
    };
  };
  ...
  function datasetsFor(varName, mode){
    ...
    // P.E. específicos da variedade
    const pe = peForVar(varName);
    ds.push(mkLine(`PE/Colheita (${pe.colheita.toFixed(2)})`, new Array(over.length).fill(pe.colheita), '#F0E442', {
      borderDash:[4,4], pointRadius:0, borderWidth:2.5, endLabel:true, endLabelText:'PEC'
    }));
    ds.push(mkLine(`PE/Beneficiado (${pe.beneficiado.toFixed(2)})`, new Array(over.length).fill(pe.beneficiado), '#999999', {
  ...
  function datasetsResumo(varName){
    ...
    return [
      ...
      mkLine(`PE/Beneficiado (${pe.beneficiado.toFixed(2)})`, new Array(atualArr.length).fill(pe.beneficiado), '#999999', {
        borderDash:[6,4], pointRadius:0, borderWidth:2.5, endLabel:true, endLabelText:'PEB'
      })
    ];
  }
  ...
  function createCard(name){
    ...
    h3.className='var-title';  // título grande e destacado
    h3.textContent=name;
...
// (estes já foram declarados antes e mantêm os listeners)
varsModalFrom?.addEventListener('change', previewDateFromModal);
varsModalTo  ?.addEventListener('change', previewDateFromModal);
varsModalFrom?.addEventListener('keydown', (e)=>{ if (e.key === 'Enter') { e.preventDefault(); } });
varsModalTo  ?.addEventListener('keydown', (e)=>{ if (e.key === 'Enter') { e.preventDefault(); } });
