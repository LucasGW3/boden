...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    // ---------- Observações por sessão (novas) ----------
    $obs_comercial = trim($_POST['obs_comercial'] ?? '');
    $obs_logistica = trim($_POST['obs_logistica'] ?? '');
    $obs_qualidade = trim($_POST['obs_qualidade'] ?? '');
    $obs_producao  = trim($_POST['obs_producao']  ?? '');
    $obs_fazenda   = trim($_POST['obs_fazenda']   ?? '');

    // ---------- LOGÍSTICA (AJUSTADO: REPEPIDOR) ----------
    $log_norm = [];
    $tmt_ls_hhmm = $tmt_truck_hhmm = '';
    $tmt_ls_min = $tmt_truck_min = 0;

    if ($can['logistica']) {
      $allowed_log = ['carreta_ls','truck'];
      $log_rows = json_decode($_POST['l_transporte'] ?? '[]', true) ?: [];

      foreach ($log_rows as $r) {
        $tipo = strtolower(trim($r['tipo'] ?? ''));
        if (!in_array($tipo, $allowed_log, true)) continue;
        $hhmm = preg_replace('/[^0-9:]/', '', $r['hhmm'] ?? '');
        $min  = hhmm_to_minutes($hhmm) ?? null;
        $log_norm[] = ['tipo'=>$tipo, 'hhmm'=>$hhmm, 'min'=>$min];
      }

      // Preenche campos legados como antes (preferindo Carreta LS)
      $prefLog = first_by_tipo($log_norm, ['carreta_ls','truck']);
      if ($prefLog) {
        if ($prefLog['tipo'] === 'carreta_ls') {
          $tmt_ls_hhmm = $prefLog['hhmm'] ?? '';
          $tmt_ls_min  = hhmm_to_minutes($tmt_ls_hhmm) ?? 0;
        } elseif ($prefLog['tipo'] === 'truck') {
          $tmt_truck_hhmm = $prefLog['hhmm'] ?? '';
          $tmt_truck_min  = hhmm_to_minutes($tmt_truck_hhmm) ?? 0;
        }
      }

      // Caso existam ambos tipos, tenta preencher também o outro a partir do primeiro registro daquele tipo
      $firstLS = first_by_tipo(array_values(array_filter($log_norm, fn($x)=>$x['tipo']==='carreta_ls')), ['carreta_ls']);
      $firstTR = first_by_tipo(array_values(array_filter($log_norm, fn($x)=>$x['tipo']==='truck')), ['truck']);
      if ($firstLS) { $tmt_ls_hhmm = $firstLS['hhmm'] ?? $tmt_ls_hhmm; $tmt_ls_min = hhmm_to_minutes($tmt_ls_hhmm) ?? $tmt_ls_min; }
      if ($firstTR) { $tmt_truck_hhmm = $firstTR['hhmm'] ?? $tmt_truck_hhmm; $tmt_truck_min = hhmm_to_minutes($tmt_truck_hhmm) ?? $tmt_truck_min; }
    }
...
$prefill = [
  // [NOVO] Logística — repetidor
  'l_transporte' => (function() use ($viewPayload){
      $rows = dot($viewPayload,'logistica.transporte_multi');
      if (is_array($rows) && $rows) return $rows;
      // fallback a partir do legado
      $ls = dot($viewPayload,'logistica.transporte.carreta_ls.hhmm') ?: (dot($viewPayload,'logistica.tempo_transporte_hhmm') ?? '');
      $tr = dot($viewPayload,'logistica.transporte.truck.hhmm') ?: '';
      $out = [];
      if ($ls) $out[] = ['tipo'=>'carreta_ls','hhmm'=>$ls,'min'=>hhmm_to_minutes($ls)];
      if ($tr) $out[] = ['tipo'=>'truck','hhmm'=>$tr,'min'=>hhmm_to_minutes($tr)];
      return $out;
  })(),
...
// [NOVO] Prefill das observações por sessão
$prefill['obs_comercial'] = dot($viewPayload,'comercial.observacoes') ?? '';
$prefill['obs_logistica'] = dot($viewPayload,'logistica.observacoes') ?? '';
$prefill['obs_qualidade'] = dot($viewPayload,'qualidade.observacoes') ?? '';
$prefill['obs_producao']  = dot($viewPayload,'producao.observacoes') ?? '';
$prefill['obs_fazenda']   = dot($viewPayload,'fazenda.observacoes') ?? '';

// Prefill para JS
$savedRepeaterData = [
  // [NOVO] logística
  'lTransporte'   => $prefill['l_transporte'],
    ...
    <form method="POST" id="safraForm" class="space-y-8" autocomplete="off">
      ...
      <section id="tab-comercial" class="tab-pane hidden">
        <div class="bg-brand-surface border border-brand-line rounded-2xl p-6 lg:p-8 space-y-6">
          ...
          <!-- NOVO: Observações — Comercial -->
          <div class="pt-2">
            <label class="block text-sm font-medium text-brand-muted mb-2">Observações — Comercial</label>
            <textarea name="obs_comercial" rows="2"
              class="block w-full px-4 py-2 bg-white border border-brand-line rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary"
              placeholder="Observações específicas do Comercial..."><?php echo htmlspecialchars($prefill['obs_comercial']); ?></textarea>
          </div>
        </div>
      </section>
      <?php endif; ?>

      <!-- Logística (AJUSTADO PARA MÚLTIPLAS ENTRADAS) -->
      ...
      <section id="tab-logistica" class="tab-pane hidden">
        <div class="bg-brand-surface border border-brand-line rounded-2xl p-6 lg:p-8 space-y-6">
          ...
          <?php
            $rowsL = $prefill['l_transporte'];
            $okL = has_value($rowsL);
          ?>
          <div class="flex items-center mb-3 gap-3">
            <label class="text-sm font-medium text-brand-muted flex items-center gap-2">
              Tempo de Transporte Fazenda > Industrial
              <span class="inline-flex items-center">
                <?php echo badge($okL); ?>
                <button type="button" class="add-row text-xs ml-2 px-3 py-1 rounded-full bg-white border border-brand-line text-brand-text hover:bg-brand-bg hover:border-brand-primary/50 transition-colors" data-target="lTransporte">Adicionar</button>
              </span>
            </label>
          </div>
          <?php echo render_rows_compact(
            array_map(function($r){ return [
              'tipo' => ($r['tipo'] ?? '') === 'carreta_ls' ? 'Carreta LS' : (($r['tipo'] ?? '')==='truck'?'Truck':($r['tipo']??'')),
              'hhmm' => $r['hhmm'] ?? ''
            ]; }, $rowsL),
            [ 'tipo'=>'Tipo', 'hhmm'=>'Tempo (HH:MM)' ]
          ); ?>
          <div id="lTransporte" class="space-y-3 mt-3"></div>
          <input type="hidden" name="l_transporte" id="l_transporte_json" />
          <p class="text-xs text-brand-muted">Adicione quantas cargas forem necessárias.</p>

          <!-- NOVO: Observações — Logística -->
          <div class="pt-2">
            <label class="block text-sm font-medium text-brand-muted mb-2">Observações — Logística</label>
            <textarea name="obs_logistica" rows="2"
              class="block w-full px-4 py-2 bg-white border border-brand-line rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary"
              placeholder="Observações específicas da Logística..."><?php echo htmlspecialchars($prefill['obs_logistica']); ?></textarea>
          </div>
        </div>
      </section>
      <?php endif; ?>
      ...
      <section id="tab-qualidade" class="tab-pane hidden">
        <div class="bg-brand-surface border border-brand-line rounded-2xl p-6 lg:p-8 space-y-8">
          ...
          <!-- NOVO: Observações — Qualidade -->
          <div class="pt-2">
            <label class="block text-sm font-medium text-brand-muted mb-2">Observações — Qualidade</label>
            <textarea name="obs_qualidade" rows="2"
              class="block w-full px-4 py-2 bg-white border border-brand-line rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary"
              placeholder="Observações específicas da Qualidade..."><?php echo htmlspecialchars($prefill['obs_qualidade']); ?></textarea>
          </div>
        </div>
      </section>
      <?php endif; ?>
      ...
      <section id="tab-producao" class="tab-pane hidden">
        <div class="bg-brand-surface border border-brand-line rounded-2xl p-6 lg:p-8 space-y-8">
          ...
          <!-- NOVO: Observações — Produção -->
          <div class="pt-2">
            <label class="block text-sm font-medium text-brand-muted mb-2">Observações — Produção</label>
            <textarea name="obs_producao" rows="2"
              class="block w-full px-4 py-2 bg-white border border-brand-line rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary"
              placeholder="Observações específicas da Produção..."><?php echo htmlspecialchars($prefill['obs_producao']); ?></textarea>
          </div>
        </div>
      </section>
      <?php endif; ?>
      ...
      <section id="tab-fazenda" class="tab-pane hidden">
        <div class="bg-brand-surface border border-brand-line rounded-2xl p-6 lg:p-8 space-y-8">
          ...
          <!-- NOVO: Observações — Fazenda -->
          <div class="pt-2">
            <label class="block text-sm font-medium text-brand-muted mb-2">Observações — Fazenda</label>
            <textarea name="obs_fazenda" rows="2"
              class="block w-full px-4 py-2 bg-white border border-brand-line rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary"
              placeholder="Observações específicas da Fazenda..."><?php echo htmlspecialchars($prefill['obs_fazenda']); ?></textarea>
          </div>
        </div>
      </section>
      <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const repeaters = {
    // NOVO: Logística multi
    lTransporte: bindRepeater('lTransporte','l_transporte_json',[
      { name:'tipo', label:'Tipo', type:'select', options: TIPOS_LOG },
    ...
    fBigBagVar: bindRepeater('fBigBagVar','f_bigbag_variedade_json',[
      { name:'variedade',   label:'Variedade', type:'select', options: VARIEDADES },
      { name:'bigbag_dia',  label:'Big bag / dia', type:'number', attrs:{ step:'any', min:'0' } },
    ]),
  };
  ...
  // Inicialização com dados gravados (inclui logística nova)
  (initialData.lTransporte||[]).forEach(r => document.getElementById('lTransporte') && repeaters.lTransporte.add(r));
  ...
  // [NOVO] Sincroniza o hidden ref_date com o campo de filtro #view
  const refHidden = document.getElementById('ref_date_hidden');
  const viewInput = document.getElementById('view');
  if (refHidden && viewInput) {
    // inicial
    if (viewInput.value) refHidden.value = viewInput.value;
    // ao mudar o filtro de data, acompanha
    viewInput.addEventListener('change', ()=>{ if (viewInput.value) refHidden.value = viewInput.value; });
  }

  document.getElementById('safraForm').addEventListener('submit', ()=>{
    Object.values(repeaters).forEach(rep => rep.snap && rep.snap());
    // redundância: antes de enviar, garante ref_date atualizado
    if (refHidden && viewInput && viewInput.value) refHidden.value = viewInput.value;
  });
});
</script>
</body>
